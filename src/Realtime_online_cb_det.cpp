...
int main(int argc, char** argv)
{
    ...
    if( capture )
    {
		...
		cvNamedWindow("新三帧差分", 1);
		cvMoveWindow("新三帧差分", 350, 0);
        ...
        for(;;)
        {
        	...
        	if( rawImage )
        	{
				...
				//cvAddWeighted(result, 0.5, pFrImg2, 0.5, 0.0, kk);
				...
				bitwise_and(result, pFrImg2, kk);
			
				cvShowImage("新三帧差分", kk);
